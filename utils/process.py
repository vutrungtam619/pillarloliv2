...
def bounding_bboxes(image_points):
    '''
    Args:
        image_points [np.ndarray float32, (m, n, 2)]: points in image coordinate
        
    Returns:
        bboxes [np.ndarray float32]
    '''
    x_min, y_min = image_points.min(axis=1).T
    x_max, y_max = image_points.max(axis=1).T
    bboxes = np.stack([
        np.column_stack([x_min, y_min]),  # top-left
        np.column_stack([x_max, y_min]),  # top-right
        np.column_stack([x_max, y_max]),  # bottom-right
        np.column_stack([x_min, y_max]),  # bottom-left
    ], axis=1)
    return bboxes