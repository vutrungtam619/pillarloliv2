...
def main(args):
    ...
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        ...
        max_momentum=0.95, div_factor=20
    )
    ...
    if checkpoint_exist:
        ...
        print(f"Found checkpoint, continue from epoch {start_epoch}")
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, data_dict in enumerate(tqdm(train_dataloader)):
            ...
            batched_bbox_reg = batched_bbox_reg.to(dtype=bbox_pred.dtype, device=bbox_pred.device)
        ...
        if (epoch + 1) % args.ckpt_freq == 0:
            ...
            ckpt_file = os.path.join(args.checkpoint_dir, f'epoch_{epoch+1}.pth')
